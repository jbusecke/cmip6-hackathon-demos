...
from preprocessing import cmip6_homogenization, cmip6_renaming_dict
...
def merge_variables_on_staggered_grid(data_dict, modelname,
                                      tracer_ref='thetao',
                                      u_ref='uo',
                                      v_ref='vo',
                                      plot=True,
...
def recreate_metrics(ds, grid):
    ...
    axis_vel_pos = {axis: list(set(grid.axes[axis].coords.keys()) - set(['center']))[0] for axis in ['X', 'Y']}
    
    # infer dx at eastern bound from tracer points
    lon0, lon1 = grid.axes["X"]._get_neighbor_data_pairs(ds.lon.load(), axis_vel_pos['X'])
    lat0 = lat1 = ds.lat.load().data
    dx = distance(lon0, lat0, lon1, lat1)
    ds.coords["dxe"] = xr.DataArray(
        dx, coords=grid.interp(ds.lon, "X").coords
    )
